...
void main() {
  group('Currency Performance Tests', () {
    setUp(() {
      ...
      CamSplitCurrencyService.clearCache();
    });
    ...
    group('Currency Service Performance', () {
      test('should cache currency lookups for improved performance', () {
        ...
        stopwatch.start();
        final currency1 = CamSplitCurrencyService.getCurrencyByCode('USD');
        stopwatch.stop();
        ...
        stopwatch.reset();
        stopwatch.start();
        final currency2 = CamSplitCurrencyService.getCurrencyByCode('USD');
        stopwatch.stop();
        ...
        final stats = CamSplitCurrencyService.getCacheStats();
      ...
      test('should handle cache eviction when max size is reached', () {
        ...
        for (int i = 0; i < 10; i++) {
          for (final code in supportedCurrencies) {
            CamSplitCurrencyService.getCurrencyByCode(code);
          }
        }
    ...
    group('Currency Formatting Performance', () {
      test('should format amounts efficiently', () {
        ...
        for (final amount in amounts) {
          CamSplitCurrencyService.formatAmount(amount, currency);
        }
      ...
      test('should handle large numbers efficiently', () {
        ...
        for (int i = 0; i < 100; i++) {
          CamSplitCurrencyService.formatAmount(largeAmount, currency);
        }
    ...
    group('Memory Usage Tests', () {
      test('should not cause memory leaks with repeated lookups', () {
        final initialStats = CamSplitCurrencyService.getCacheStats();
        
        // Perform many lookups
        for (int i = 0; i < 1000; i++) {
          CamSplitCurrencyService.getCurrencyByCode('USD');
        }
        
        final finalStats = CamSplitCurrencyService.getCacheStats();
      ...
      test('should handle cache clearing efficiently', () {
        ...
        final statsBefore = CamSplitCurrencyService.getCacheStats();
        expect(statsBefore['cacheSize'], greaterThan(0));
        
        // Clear cache
        CamSplitCurrencyService.clearCache();
        
        final statsAfter = CamSplitCurrencyService.getCacheStats();
